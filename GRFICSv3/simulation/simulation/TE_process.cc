...
void ensure_alarm_dir() {
    struct stat st {};
    if (stat("/var/log/grfics/alarms", &st) == 0 && S_ISDIR(st.st_mode)) {
        return;
    }
    mkdir("/var/log/grfics", 0755);
    mkdir("/var/log/grfics/alarms", 0755);
}

/* Append JSONL lines for SIEM / Filebeat (process_alarm log_type). Throttled to avoid flooding. */
void append_process_alarm_bootstrap(double pressure_kpa) {
    ...
    ensure_alarm_dir();
    FILE *f = std::fopen("/var/log/grfics/alarms/reactor_pressure.jsonl", "a");
    if (!f) {
        return;
    }
    std::fprintf(
        f,
        "{\"ts\":%lld,\"alarm\":\"PROCESS_STATE\",\"severity\":0,\"pressure_kpa\":%.2f,\"unit\":\"kPa\","
        "\"subsystem\":\"TE\",\"phase\":\"bootstrap\"}\n",
        static_cast<long long>(std::time(nullptr)),
        pressure_kpa);
    std::fclose(f);
}

void maybe_log_pressure_alarm(double pressure_kpa) {
    static int last_level = 0;
    static std::time_t last_emit = 0;
    const std::time_t now = std::time(nullptr);

    int level = 0;
    /* Align with Chain 1 lab runs: excursions often reach high-2700s before 2800 kPa. */
    if (pressure_kpa >= 3150.0) {
        level = 3;
    } else if (pressure_kpa >= 3000.0) {
        level = 2;
    } else if (pressure_kpa >= 2720.0) {
        level = 1;
    }
    ...
    /* At least 2s between lines; always log on severity increase */
    if (level <= last_level && (now - last_emit) < 2) {
        return;
    }
    last_level = level;
    last_emit = now;
    ...
    std::fprintf(
        f,
        "{\"ts\":%lld,\"alarm\":\"REACTOR_PRESSURE\",\"severity\":%d,\"pressure_kpa\":%.2f,\"unit\":\"kPa\",\"subsystem\":\"TE\"}\n",
...
TE::TE() {
    ...
    /* One JSONL line so Filebeat / SIEM always have a file-backed process_alarm stream (severity 0). */
    append_process_alarm_bootstrap(pressure);
}
...
void TE::update(Json::Value inputs) {
    ...
    maybe_log_pressure_alarm(pressure);
}
